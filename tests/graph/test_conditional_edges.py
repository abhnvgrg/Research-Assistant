"""
Conditional edge function tests — these are pure functions of state
that determine graph routing, so they're trivially unit-testable
without touching the graph itself.

route_after_reflection is the most important: it's the single guard
against edge case 5.1 (reflection thrashing / runaway cost).
"""
...
def test_check_decomposition_fails_on_no_topic(decomposed_state):
    """Edge case 1.1 — semantically empty query."""
...
def test_check_decomposition_fails_on_multi_intent(decomposed_state):
    """Edge case 1.3 — two unrelated topics fused in one query."""
...
# ---- route_after_reflection (the critical anti-thrashing guard) ----
...
def test_route_after_reflection_hard_caps_at_max_cycles(base_state):
    """This is THE guard against reflection thrashing (edge case
    5.1) and its associated 2.7x cost blowup (cost modeling section).
    Even if reflection keeps failing, MAX_CYCLES must force exit."""
...
def test_route_after_reflection_caps_even_with_many_fails_beyond_max(base_state):
    base_state["cycle_count"] = MAX_CYCLES + 5  # defensive: should never happen, but must not loop forever
...
def test_route_after_reflection_defaults_to_pass_with_empty_history(base_state):
    """Defensive: if reflection_history is somehow empty (e.g. the
    reflection node itself crashed before appending), don't loop
    forever — treat as a pass and exit."""
    base_state["cycle_count"] = 1
    base_state["reflection_history"] = []
    assert route_after_reflection(base_state) == "formatter"
